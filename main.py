...
async def create_upload_files(files: List[UploadFile] = File(...)):
    ...
    for image, name in zip(images_rgb, names):
        ...
        pillow_image.save('static/resize-images' + name)
    
    image_paths = ['static/resize-images' + name for name in names]
...
async def main():
    ...
    full_paths = ['static/images/' + path for path in paths]
                  
    scenes_names = ['Building', 'Forest', 'Glacier', 'Mountain', 'Sea', 'Street']
    
    column_lables = []
    
    content = content + get_html_table(full_paths, scenes_names, column_lables)
